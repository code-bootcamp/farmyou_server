...
export class AdminService {
  ...
  async create({ email, hashedPassword: password, directStoreId, isWebMaster }) {
    if (isWebMaster === true) {
      const webmaster = await this.adminRepository.findOne({isWebMaster: true});
    ...
    try {
      ...
      throw new ConflictException("해당 직매장에 관리자를 배치할 수 없습니다.");
    }
  }
